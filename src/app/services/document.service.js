...
let DocumentService = (() => {
    ...
    var DocumentService = _classThis = class {
        constructor(http) {
            this.http = http;
            this.apiUrl = 'http://localhost:8080/documents/create';
            this.updateUrl = 'http://localhost:8080/documents/update';
        ...
        // }
        // Save document
        saveDocument(title, content, owner) {
        ...
        updateDocument(title, content, docId) {
            const headers = new http_1.HttpHeaders({
                Authorization: 'Basic ' + btoa(`${this.username}:${this.password}`)
            });
